...
.controller('View2Ctrl', ['$scope', '$log', '$location', function($scope, $log, $location) {
	...
	var blogContent = [
		...
		"Watch the moment when Sir David Attenborough and BBC filmmakers recorded chunks of ice falling from a glacier. Even better, you can watch a further 1,000 more memorable moments, for free, anytime, on your smartphone or tablet, via Attenborough \'s Story of Life app, which is now available to download via Google Play",
	...
	$scope.removeIt = function(nowCount) {
		for (var i = 0, j=0; i < $scope.serialCount && j<$scope.blogArray.length; i++) {
			if($scope.blogArray[j++].presentCount == nowCount) {
